...
class ErrorHandling(Cog):
    ...
    async def on_command_error(self, ctx: Context, error):
        ...
        if isinstance(error, ignored):
        ...
        elif isinstance(error, ActionCancelled):
            embed = Embed(title="🚫 Action Cancelled!", color=Colours.soft_red)
            ...
            await ctx.reply(f':hourglass: | Cooldown: `{error.retry_after:,.0f}` seconds.')
