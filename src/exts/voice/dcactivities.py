...
from discord.ext.commands import Cog, Context, command, cooldown, cooldowns
...
class ActivityCog(Cog):
    ...
    @command(name="poker", aliases=["poker_game", "poker_night"])
    ...
    @command(name="fishing", aliases=["hangman_game"])
    @cooldown(1, 60, cooldowns.BucketType.user)
    async def hangman_game(self, ctx: Context, channel: Optional[VoiceChannel] = None):
        """Command used to play hangman within 2 players on Voice Channel"""
        if not channel:
        ...
        invite = await self.bot.dcactivity.create_invite(
            _channel, DCApplication.fishing, max_age=86400, max_uses=10)

        embed = Embed(title="Play Hangman Together!", color=Colours.soft_red)
        embed.add_field(name="Invitation Link :", value=invite)
        embed.set_image(url="https://shorturl.at/gGQS2")
        embed.set_footer(text="This link will expire in 24 hours")
